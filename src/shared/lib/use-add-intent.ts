...
/**
 * Читает `?open=<id>` — «открой вот это, а не создавай новое».
 *
 * Брат-близнец useAddIntent по механике (однократный захват при монтировании,
 * очистка адреса эффектом) и его противоположность по смыслу. Разделены
 * намеренно: `?add=` означает «открой пустую форму», а карточка «Что изучить»
 * на экране пути ведёт в конкретный урок академии и конкретную книгу
 * библиотеки — это «открой существующее», и складывать два разных намерения в
 * один параметр значило бы, что принимающий экран должен угадывать, чего от
 * него хотят, по формату значения.
 *
 * Возвращает id или null. Проверять, что такой id существует, обязан
 * вызывающий: адрес приходит снаружи и может содержать что угодно.
 */
export function useOpenIntent(): string | null {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
