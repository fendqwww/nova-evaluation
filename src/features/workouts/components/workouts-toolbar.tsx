 ...
 * Habits and Tasks use, so moving between sections never means relearning the
 * header.
 *
 ...
 */
export function WorkoutsToolbar({
  query,
  onQueryChange,
  sort,
  onSortChange,
  ...
  sort: WorkoutSortId;
  onSortChange: (value: WorkoutSortId) => void;
  filter: WorkoutFilterId;
  onFilterChange: (value: WorkoutFilterId) => void;
  counts: Record<WorkoutFilterId, number>;
}) {
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center gap-2">
        ...
        <div className="relative shrink-0">
          ...
          <select
            value={sort}
            onChange={(event) => onSortChange(event.target.value as WorkoutSortId)}
