...
/**
 * Search, sort and filter in two dense rows.
 *
 ...
 * ("3 просроченных") rather than a set of blind buttons.
 */
export function GoalsToolbar({
  ...
  sort: GoalSortId;
  onSortChange: (value: GoalSortId) => void;
  filter: GoalFilterId;
  onFilterChange: (value: GoalFilterId) => void;
  counts: Record<GoalFilterId, number>;
}) {
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center gap-2">
        ...
        {/* A native select on purpose: it opens the platform picker on mobile,
            which beats any custom sheet for a four-option choice. `color-scheme:
            dark` on <html> is what keeps the popup dark. */}
        <div className="relative shrink-0">
          ...
          <select
            value={sort}
            onChange={(event) => onSortChange(event.target.value as GoalSortId)}
