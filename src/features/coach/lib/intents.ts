...
export const COACH_QUICK_ACTIONS: CoachQuickAction[] = [
  ...
  // Два вопроса не о себе, а о предмете — и стоят они здесь намеренно. Все
  // остальные чипы спрашивают «как у меня дела», и человек, читая их, делает
  // единственно возможный вывод: коуч умеет говорить только про его цифры.
  // Он умеет больше (см. блок scope в ai/prompts/coach.ts), но узнать об этом
  // было неоткуда. Эти два показывают вторую половину работы тренера.
  { intent: "workouts", label: "Как наращивать нагрузку?" },
  { intent: "nutrition", label: "Сколько мне нужно белка?" },
];
