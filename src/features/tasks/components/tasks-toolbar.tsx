 ...
 * and Habits use, so moving between sections never means relearning the header.
 *
...
export function TasksToolbar({
  ...
  sort: TaskSortId;
  onSortChange: (value: TaskSortId) => void;
  filter: TaskFilterId;
  onFilterChange: (value: TaskFilterId) => void;
  counts: Record<TaskFilterId, number>;
}) {
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center gap-2">
        ...
        <div className="relative shrink-0">
          ...
          <select
            value={sort}
            onChange={(event) => onSortChange(event.target.value as TaskSortId)}
